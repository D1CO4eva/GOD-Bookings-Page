...
import { DEFAULT_API_BASE } from './programCatalog';

export interface BookingApiClientConfig {
  apiBase?: string;
...
const extractBookings = (data: unknown): BookingRecord[] => {
  ...
  if (headerRow) {
    ...
    programCol = headerStrings.findIndex(
      (cell) =>
        cell.includes('type of program') || cell.includes('program type') || cell.includes('program')
    );
    timeCol = headerStrings.findIndex((cell) => cell.includes('time'));
    emailCol = headerStrings.findIndex(
      (cell) => cell.includes('host email') || cell === 'email' || cell.includes('email')
    );
...
const buildVerifyReservationEndpoint = (reservationVerifyEndpoint: string, lookup: ReservationLookupData): string => {
  ...
  return query ? `${reservationVerifyEndpoint}?${query}` : reservationVerifyEndpoint;
};
...
const coerceReservationDetails = (
  ...
  const time = pickFirstString(obj.time, obj.Time, obj.programTime, obj.program_time, obj['Time Slot']);
...
export const createBookingApiClient = (config: BookingApiClientConfig = {}) => {
  const fetchImpl = config.fetchImpl || fetch;
  const apiBase = normalizeApiBase(config.apiBase || DEFAULT_API_BASE);

  const bookingsEndpoint = `${apiBase}/api/bookings`;
  const reservationVerifyEndpoint = `${apiBase}/api/reservations/verify`;
  const reservationUpdateEndpoint = `${apiBase}/api/reservations/update`;
  const reservationDeleteEndpoint = `${apiBase}/api/reservations/delete`;

  const fetchBookings = async (): Promise<BookingRecord[]> => {
    try {
      const response = await fetchImpl(bookingsEndpoint, {
        method: 'GET',
        cache: 'no-cache'
      });
  ...
  const submitBooking = async (data: BookingData): Promise<BookingSubmitResult> => {
    try {
  ...
  const verifyReservation = async (lookup: ReservationLookupData): Promise<ReservationLookupResult> => {
    try {
      const verifyEndpoint = buildVerifyReservationEndpoint(reservationVerifyEndpoint, lookup);
  ...
  const updateReservation = async (
    ...
    try {
      ...
      const response = await fetchImpl(reservationUpdateEndpoint, {
        method: 'POST',
        cache: 'no-cache',
        headers: {
          'Content-Type': 'application/json'
  ...
  const cancelReservation = async (
    ...
    try {
      ...
      const response = await fetchImpl(reservationDeleteEndpoint, {
        method: 'POST',
        cache: 'no-cache',
        headers: {
          'Content-Type': 'application/json'
